...
4. API 권한은 Trading Account의 Perpetual Futures Read/Write만 사용하고 Withdrawal은 비활성화합니다.
...
`.env.worker`에서 `SUPABASE_SERVICE_ROLE_KEY`, 실제 고정 `WORKER_PUBLIC_IP`를 입력합니다. 이 파일은 Git에 올리지 않습니다.
...
- 2차: 조회·계산 검증 후 `TRADING_MODE=DRY_RUN`, `RUN_READINESS_CHECK=true`
...
`UNKNOWN` 주문은 같은 Delta를 즉시 다시 보내지 않습니다. Gate 주문 ID 또는 고유 주문 `text`로 조회해 결과가 확인된 뒤에만 다음 동기화를 진행합니다.
